...
static int _gb_sdio_process_events(struct gb_sdio_host *host, u8 event)
{
    ...
    //dev_info(mmc_dev(host->mmc), "_gb_sdio_process_events() \n");

    //dev_info(mmc_dev(host->mmc), "host->mmc->caps = 0x%x \n", host->mmc->caps);

	if (event & GB_SDIO_CARD_INSERTED) {
        //dev_info(mmc_dev(host->mmc), "GB_SDIO_CARD_INSERTED... \n");
        //dev_info(mmc_dev(host->mmc), "is removalbe = 0x%x \n", mmc_card_is_removable(host->mmc));
		if (!mmc_card_is_removable(host->mmc)) {
            //dev_info(mmc_dev(host->mmc), "!mmc_card_is_removalbe return\n");
			return 0;
        }
		if (host->card_present) {
            //dev_info(mmc_dev(host->mmc), "host->card_present return \n");
			return 0;
        }
        //dev_info(mmc_dev(host->mmc), "state_changed = 1 \n");
		host->card_present = true;
		state_changed = 1;
	}

	if (event & GB_SDIO_CARD_REMOVED) {
        //dev_info(mmc_dev(host->mmc), "GB_SDIO_CARD_REMOVED... \n");
		if (!mmc_card_is_removable(host->mmc)) {
            //dev_info(mmc_dev(host->mmc), "!mmc_card_is_removalbe return\n");
			return 0;
        }
	...
	if (event & GB_SDIO_WP) {
        //dev_info(mmc_dev(host->mmc), "GB_SDIO_WP... \n");
		host->read_only = true;
	}
    ...
    //dev_info(mmc_dev(host->mmc), "_gb_sdio_process_event() return 0 \n");
...
static int gb_sdio_event_recv(u8 type, struct gb_operation *op)
{
    ...
    //dev_info(mmc_dev(host->mmc), "gb_sdio_event_recev() \n");
    ...
    //dev_info(mmc_dev(host->mmc), "GB_SDIO_TYPE_EVENT PASS \n");
    ...
    //dev_info(mmc_dev(host->mmc), "payload size PASS \n");
			   ...
			   struct gb_sdio_set_ios_request *request)
{
    ...
    //dev_info(mmc_dev(host->mmc), "gb_sdio_set_ios() \n");
...
static int gb_sdio_transfer(struct gb_sdio_host *host, struct mmc_data *data)
{
    ...
    //dev_info(mmc_dev(host->mmc), "gb_sdio_transfer() \n");
    
    //dev_info(mmc_dev(host->mmc), "caps = 0x%x \n", host->mmc->caps);
...
static int gb_sdio_command(struct gb_sdio_host *host, struct mmc_command *cmd)
{
    ...
    //dev_info(mmc_dev(host->mmc), "gb_sdio_command() \n");
    ...
    //dev_info(mmc_dev(host->mmc), "cmd->arg = 0x%x \n", cmd->arg);
    ...
    //dev_info(mmc_dev(host->mmc), "request.cmd = >>>>>> [%d] \n", request.cmd);
    //dev_info(mmc_dev(host->mmc), "request.cmd_flags = 0x%x \n", request.cmd_flags);
    //dev_info(mmc_dev(host->mmc), "request.cmd_type = 0x%x \n", request.cmd_type);
    //dev_info(mmc_dev(host->mmc), "request.cmd_arg = 0x%x \n", request.cmd_arg);

    if(host->mrq->data) {
        struct mmc_data *data = host->mrq->data;
        request.data_cmd = 1;
        request.data_blocks = data->blocks;
        request.data_blksz = data->blksz;
    }
    else {
        request.data_cmd = 0;
    }
	...
	if (cmd_flags & GB_SDIO_RSP_R2) {
		for (i = 0; i < 4; i++) {
            ...
            //dev_info(mmc_dev(host->mmc), "cmd->resp[%d] = 0x%08x \n", i, cmd->resp[i]);
        }
    }
	else {
		cmd->resp[0] = le32_to_cpu(response.resp[0]);
        //dev_info(mmc_dev(host->mmc), "cmd->resp[0] = 0x%08x \n", cmd->resp[0]);
    }
